...
const atualizar_telefone = async (req: NextApiRequest, res: NextApiResponse) => {
  const { body: { telefone } } = req
  if (req.method === 'POST') {
    ...
    const processo_de_atualizacao = new Promise(async (resolve, reject) => {
      ...
      const telefone_atualizado = await
        REPO.REGISTRO.atualizar_telefone(
          registro.DAN, telefone)

      if (!telefone_atualizado)
        resolve({ status: 'ERRO', erro: 'Serviço indisponível!' })

      resolve({ status: 'ATUALIZADO', registro: telefone_atualizado })
    })
...
export default atualizar_telefone