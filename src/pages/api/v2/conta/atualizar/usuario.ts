...
const atualizar_usuario = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  if (req.method === 'POST') {
    ...
    const processo_de_atualizacao = new Promise(async (resolve, reject) => {
      ...
      const retorno_acesso = await REPO.REGISTRO.DAN_com_senha(registro.DAN)

      if (!retorno_acesso.email_verificado) {
        resolve({ status: 'ERRO', erro: 'Email não verificado, verifique-o antes!' })
      }

      const usuario_atualizado = await
        REPO.REGISTRO.atualizar_dados_de_seguranca_usuario(
          registro.DAN, usuario)

      if (!usuario_atualizado)
        resolve({ status: 'ERRO', erro: 'Serviço indisponível!' })

      resolve({ status: 'ATUALIZADO', registro: usuario_atualizado })
    })
...
export default atualizar_usuario